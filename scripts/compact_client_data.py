...
def compact_full(src):
    out={
        'schema':src.get('schema'),'version':VERSION,'format':FORMAT,
        'source':src.get('source'),'updated_at':src.get('updated_at'),
        'errors':src.get('errors',[]),'validation':src.get('validation',{}),
        'store_catalog':[],'days':[]
    }
    catalog_index={}
    ...
    for d in src.get('days',[]):
        ...
        row={
            'date':d.get('date'),'url':d.get('url'),'label':d.get('label'),'status':d.get('status'),
            'rates':d.get('rates',[]),'offers':[],
            'diagnostics':{
                ...
                'count_warnings':len(diag.get('count_warnings',[]))
            }
        }
        ...
        row['diagnostics']['stores_unique']=len(row['offers'])
        out['days'].append(row)
    return out
...
def main(argv=None):
    ...
    inp=args.input; outp=args.output or inp
    src=json.loads(inp.read_text(encoding='utf-8')); out=compact(src)
