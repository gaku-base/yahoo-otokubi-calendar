...
async def audited_select_meta(page):
    ...
    for m in metas:
        ...
        if unparsed:
            issues.append({'select_index':m.get('index'),'rate':m.get('rate'),'kind':'unparsed_category_options','options':[base.clean(o.get('text',''))[:120] for o in unparsed[:20]]})
        if len(opts)>1 and not parsed:
            issues.append({'select_index':m.get('index'),'rate':m.get('rate'),'kind':'no_parseable_category_counts','options':[base.clean(o.get('text',''))[:120] for o in opts[:20]]})
    _AUDIT[id(page)]={'rate_sections':rate_sections,'issues':issues}
    return metas
...
async def collect_bonus(page):
    ...
    events.sort(key=lambda x:x[0]);out['list_diagnostics'].update({'event_links':len(events),'concurrency':CONCURRENCY,'strict_count_warning_gate':True,'category_option_audit':True})
...
async def main():
    ...
    counts=dict(base_validation.get('counts',{}));counts.update({'campaigns':len(rows),'campaign_dates':len({d for c in rows for d in c.get('dates',[])}),'elapsed_seconds':round(time.monotonic()-started,1),'concurrency':CONCURRENCY,'category_audit_issues':audit_issues})
