...
async def collect_event(page, date, href, label):
    rec={'date':date,'url':href,'label':label,'status':'ok','stores':[],'rates':[],'diagnostics':{'sections':[],'failures':[]}}
    try:
        ...
        metas=await select_meta(page)
        metas=[m for m in metas if m.get('rate') is not None]
        initial=await dom_store_rows(page)
        allrows=list(initial)
        diag=rec['diagnostics']; diag['rate_sections']=len(metas)
        for m in metas:
            rate=float(m['rate']); sec={'rate':rate,'select_index':m['index'],'categories_total':0,'categories_succeeded':0,'categories_failed':0,'expected_stores':0,'captured_stores':0}
            options=[]
            for o in m['options']:
            ...
            for opt,expected in options:
                ...
                if expected==0:
                    sec['categories_succeeded']+=1
                    continue
                got=[]
                last_count=-1
                try:
                    for attempt in range(3):
                        await choose_option(page,m['index'],opt)
                        rows=[r for r in await dom_store_rows(page) if float(r['rate'])==rate]
                        for r in rows: r['categories']=[category]
                        got=dedupe(rows)
                        last_count=len(got)
                        if last_count==expected: break
                        await page.wait_for_timeout(500*(attempt+1))
                    if len(got)!=expected:
                        raise RuntimeError(f'expected {expected} stores, captured {len(got)}')
                    allrows.extend(got); sec['categories_succeeded']+=1; sec['captured_stores']+=len(got)
                except Exception as e:
                    sec['categories_failed']+=1
                    if len(diag['failures'])<80: diag['failures'].append({'rate':rate,'select':m['index'],'category':category,'expected':expected,'captured':max(last_count,0),'message':clean(repr(e))[:260]})
            diag['sections'].append(sec)
        stores=dedupe(allrows)
        rec['stores']=stores; rec['rates']=sorted({float(r['rate']) for r in stores},reverse=True)
        ...
        if not metas or not stores:
            ...
            rec['status']='partial'; rec['error']='one or more categories did not match Yahoo displayed store counts'
...
async def collect_bonus(page):
    ...
    out['days']=await asyncio.gather(*(one(e) for e in events))
...
async def main():
    async with async_playwright() as p:
        ...
        bonus=await collect_bonus(page)
        guide=await legacy.collect_guide(page)
        await browser.close()
    validation=validate(bonus,guide); bonus['validation']=validation; guide['validation']=validation; guide['version']=VERSION
