...
def parse_target_guide(text:str):
    ...
    for i,title in enumerate(lines):
        ...
        for j in range(i+1,min(i+28,len(lines))):
            ...
            # The next campaign normally begins immediately before a new 開催期間 line.
            if j>i+3 and lines[j].startswith('開催期間') and any(x.startswith('開催期間') for x in block):break
        ...
        if row is None:
            ...
            row={'title':canonical,'rate':rate,'rate_label':f'+{rate:g}%','is_max':False,'is_total':False,'dates':[],'conditions':[],'source':GUIDE_URL,'entry_required':'要エントリー' in conditions,'target_store_limited':target};groups[key]=row
        for iso in dates:
            if iso not in row['dates']:row['dates'].append(iso)
        for c in conditions:
            if c not in row['conditions']:row['conditions'].append(c)
        row['entry_required']=row['entry_required'] or ('要エントリー' in conditions);row['target_store_limited']=row['target_store_limited'] or target
    out=list(groups.values())
    for r in out:r['dates'].sort()
    return out
...
def merge_target_guide(rows:list[dict],guide_rows:list[dict]):
    ...
    for g in guide_rows:
        ...
        if target is None:
            ...
            target['dates']=sorted(set((target.get('dates') or [])+(g.get('dates') or [])))
            target['conditions']=list(dict.fromkeys((target.get('conditions') or [])+(g.get('conditions') or [])))
            target['entry_required']=bool(target.get('entry_required') or g.get('entry_required'))
            target['target_store_limited']=bool(target.get('target_store_limited') or g.get('target_store_limited'))
            if target.get('rate') is None:target['rate']=g.get('rate')
            if not target.get('rate_label'):target['rate_label']=g.get('rate_label')
            target['schedule_source']=GUIDE_URL
...
async def collect_target_guide(page):
    ...
    try:
        ...
        await page.wait_for_timeout(300);text=await page.locator('body').inner_text(timeout=15000);result['campaigns']=parse_target_guide(text)
        kinds={_kind_row(x) for x in result['campaigns']}
        if not {'plus2','plus3'}.issubset(kinds):raise RuntimeError(f'guide missing expected BONUS+ schedule sections: {sorted(kinds)}')
