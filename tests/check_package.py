...
html=(root/'docs/index.html').read_text(); assert html.index('core.js') < html.index('app.js'); assert 'class="workspace"' in html and 'class="sidePane"' in html; assert 'v0.8.0' in html
...
sw=(root/'docs/sw.js').read_text(); assert "SHELL_CACHE='otokubi-v080'" in sw and "DATA_CACHE='otokubi-data-v1'" in sw; assert 'core.js' in sw and 'app.js' in sw; assert "u.search=''" in sw and 'cache.put(key,res.clone())' in sw and 'cache.match(key)' in sw; assert "fetch(req,{cache:'no-store'})" in sw; assert 'self.skipWaiting()' in sw and 'self.clients.claim()' in sw
app=(root/'docs/app.js').read_text(); assert 'Promise.allSettled' in app and "fetchJson('data/status.json')" in app; assert '最新取得失敗' in app and '前回正常データを使用' in app; assert 'BONUS+更新失敗（前回データを表示）' in app and 'キャンペーン更新失敗（前回データを表示）' in app; assert 'rate_label' in app and '単純加算しません' in app
