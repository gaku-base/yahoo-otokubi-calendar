...
required=['docs/index.html','docs/styles.css','docs/core.js','docs/app.js','docs/sw.js','docs/manifest.webmanifest','docs/icon.svg','scripts/scrape.py','scripts/scrape_v066.py','scripts/scrape_v071.py','scripts/scrape_v072.py','scripts/calendar_campaigns.py','scripts/compact_client_data.py','.github/workflows/pages.yml','.github/workflows/refresh.yml','.github/workflows/smoke.yml','tests/smoke_pwa.py','tests/test_calendar_campaigns.py']
for f in required: assert (root/f).exists(),f
html=(root/'docs/index.html').read_text(); assert html.index('core.js') < html.index('app.js'); assert 'class="workspace"' in html and 'class="sidePane"' in html
...
refresh=(root/'.github/workflows/refresh.yml').read_text(); assert 'scrape_v072.py' in refresh and "d.get('version')=='0.7.2'" in refresh
pages=(root/'.github/workflows/pages.yml').read_text(); assert "d.get('version')=='0.7.2'" in pages
