...
def sample():
    return {
        ...
        'days':[{
            ...
            'diagnostics':{'categories_total':2,'categories_succeeded':2,'categories_failed':0,'stores_total':4,'count_warnings':[{'x':1}]},
            'stores':[
...
def test_indexed_compaction_deduplicates_and_keeps_max_rate():
    ...
    assert len(offers)==2
    assert out['days'][0]['diagnostics']['count_warnings']==1
    assert out['days'][0]['diagnostics']['stores_unique']==2

def test_catalog_ids_are_reused_across_days():
    src={'validation':{'ok':True},'days':[
...
def test_compaction_is_idempotent():
    first=compact(sample()); second=compact(first)
    assert second==first

def test_validate_rejects_duplicate_date_and_invalid_rate():
    out=compact(sample())
    out['days'].append(dict(out['days'][0]))
