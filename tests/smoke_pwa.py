...
def main():
    with sync_playwright() as p:
        ...
        page=desktop.new_page();page.goto(URL,wait_until='domcontentloaded');wait_loaded(page);check_sunday_start(page);check_offer(page,target_offer(page));check_known_alias(page,'Joshin','joshin');check_known_alias(page,'ヤマダ電機','yamada-denki');check_campaign_metadata(page);check_top3_ranking(page);check_auto_campaign_eligibility(page);cols=page.locator('.workspace').evaluate("e=>getComputedStyle(e).gridTemplateColumns");widths=[float(x) for x in re.findall(r'([0-9.]+)px',cols)];assert len(widths)==2 and widths[0]>widths[1]>=350,cols;desktop.close()
    ...
    print('PWA smoke: PASS (auto official campaigns + safe +2 eligibility + informational events + base 7% + Sunday-first + iPhone offline + Windows + PNG)')
