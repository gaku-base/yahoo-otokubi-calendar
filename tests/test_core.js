...
let ev=c.eventsFor('2026-08-25',{campaigns:[{title:'テスト企画',dates:['2026-08-25'],rate:7},{title:'テスト企画',dates:['2026-08-25'],rate:7}]});
assert.deepEqual(ev.map(x=>x.title),['5のつく日','テスト企画']);
assert.equal(ev[0].rate,4);assert.equal(ev[1].rate,7);
ev=c.eventsFor('2026-09-01',{campaigns:[]});const fd=ev.find(x=>x.title==='ファーストデイ');assert(fd);assert.equal(fd.rate,3);
