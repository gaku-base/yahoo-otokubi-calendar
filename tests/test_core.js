...
let ev=c.eventsFor('2026-08-25',{source:'official',campaigns:[{title:'Brand Week',dates:['2026-08-25'],rate:null,rate_label:'最大25%',is_total:true,entry_required:true,target_store_limited:true,conditions:['要エントリー 対象ストア限定']}]});
assert.equal(ev[0].title,'5のつく日');assert.equal(ev[0].rate,4);assert.equal(ev[0].rate_label,'+4%');
const bw=ev.find(x=>x.title==='Brand Week');assert(bw);assert.equal(bw.rate,null);assert.equal(bw.rate_label,'最大25%');assert.equal(bw.is_total,true);assert.equal(bw.entry_required,true);assert.equal(bw.target_store_limited,true);assert.deepEqual(bw.conditions,['要エントリー 対象ストア限定']);
