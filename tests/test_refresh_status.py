...
def test_wrapper_source_restores_good_data_and_keeps_failed_diagnostics():
    src=(S/'run_refresh_safe.py').read_text(encoding='utf-8')
    assert 'restore_bytes(bonus_path,old_bonus_bytes)' in src and 'restore_bytes(campaign_path,old_campaign_bytes)' in src
    assert "DIAG/f'{stamp}_failed_bonus.json'" in src and 'DEFAULT_COMMAND' in src and 'scrape_v075.py' in src
