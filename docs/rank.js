...
function pointsFor(rate,amount,min=0,cap=Infinity){const r=Number(rate);if(!amount||!Number.isFinite(r)||r<=0||amount<min)return 0;const p=Math.floor(amount*r/100);return Number.isFinite(cap)?Math.min(p,cap):p}
function basePointInfo(amount){
  if(!amount)return {points:0,target:0,nominalRate:BASE_PROFILE.nominalRate,store:0,line:0,lyp:0,paypay:0};
...
function monthRows(){const y=view.getFullYear(),m=view.getMonth(),days=new Date(y,m+1,0).getDate(),shop=currentShopQuery(),amount=purchaseAmount(),rows=[];for(let n=1;n<=days;n++){const row=scoreDay(isoForDay(y,m,n),n,shop,amount);if(row.rankScore>0)rows.push(row)}rows.sort((a,b)=>b.rankScore-a.rankScore||b.addRate-a.addRate||b.bonusRate-a.bonusRate||b.eventCount-a.eventCount||a.day-b.day);rows.forEach((row,i)=>row.rank=i+1);return rows}
function monthRanking(){return monthRows().slice(0,3)}
function clickedDayInfo(iso){const rows=monthRows(),ranked=rows.find(x=>x.iso===iso);if(ranked)return ranked;const y=view.getFullYear(),m=view.getMonth(),prefix=`${y}-${String(m+1).padStart(2,'0')}-`;if(!String(iso||'').startsWith(prefix))return null;const day=Number(String(iso).slice(8));if(!day)return null;return {...scoreDay(iso,day,currentShopQuery(),purchaseAmount()),rank:null}}
function ensureLegend(){const legend=document.querySelector('.legend');if(!legend||legend.querySelector('.top3Legend'))return;const s=document.createElement('span');s.className='top3Legend';s.textContent='👑1位　🥈2位　🥉3位';legend.appendChild(s)}
function ensureStrip(){const pane=document.querySelector('.calendarPane'),cal=document.querySelector('#calendar');if(!pane||!cal)return null;let strip=document.querySelector('#top3Strip');if(!strip){strip=document.createElement('section');strip.id='top3Strip';strip.className='top3Strip';strip.setAttribute('aria-label','今月のお得日トップ3');pane.insertBefore(strip,cal)}return strip}
...
function updateRankMarks(){
  ensureLegend();document.querySelectorAll('#calendar .rankMark').forEach(x=>x.remove());document.querySelectorAll('#calendar .day').forEach(x=>x.classList.remove('rank1','rank2','rank3'));
  const ranked=monthRanking(),strip=ensureStrip();if(!strip)return;strip.innerHTML='';
  if(!ranked.length){strip.classList.add('emptyTop3');strip.textContent=currentShopQuery()?'この月は順位を付けられる確認済み追加特典がありません':'ショップを選ぶと、そのショップのお得日TOP3を表示します';return}
  strip.classList.remove('emptyTop3');ranked.forEach((r,i)=>{const cfg=RANKS[i],cell=dayButton(r.day);if(cell){cell.classList.add(cfg.cls);const mark=document.createElement('span');mark.className=`rankMark ${cfg.cls}`;mark.textContent=i===0?'👑1':i===1?'🥈2':'🥉3';mark.setAttribute('aria-label',cfg.label);cell.appendChild(mark)}const b=document.createElement('button');b.type='button';b.className=`top3Item ${cfg.cls}`;const amountMode=r.amount>0,main=amountMode?`約${Math.round(r.totalPoints).toLocaleString('ja-JP')}pt`:`合計${fmtRate(r.rateScore)}%`,sub=amountMode?`合計実質${fmtRate(Math.round(r.effectiveRate*10)/10)}%`:`基本7% + 特典${fmtRate(r.addRate)}%`;b.innerHTML=`<b>${i===0?'👑':i===1?'🥈':'🥉'} ${cfg.label}</b><span>${r.day}日</span><strong>${main}</strong><small>${sub}</small>`;b.title=amountMode?`${r.iso} 基本分を含む概算獲得 約${Math.round(r.totalPoints).toLocaleString('ja-JP')}pt`:`${r.iso} 基本7% + 確認済み特典${fmtRate(r.addRate)}%`;b.onclick=()=>{const target=dayButton(r.day);if(target)target.click()};strip.appendChild(b)})
}
function refreshForAmount(){if(typeof refreshSelectedDetail==='function')refreshSelectedDetail();else render()}
const amountInput=document.querySelector('#purchaseAmount');if(amountInput){const saved=localStorage.getItem('purchaseAmount');if(saved&&Number(saved)>0)amountInput.value=saved;let t;amountInput.addEventListener('input',()=>{const n=purchaseAmount();if(n)localStorage.setItem('purchaseAmount',String(Math.round(n)));else localStorage.removeItem('purchaseAmount');clearTimeout(t);t=setTimeout(refreshForAmount,80)})}
