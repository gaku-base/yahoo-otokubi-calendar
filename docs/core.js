(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.OtokubiCore=api;})(typeof self!=='undefined'?self:this,function(){
  ...
  function rowsFor(day,catalog){if(Array.isArray(day?.stores))return day.stores;if(Array.isArray(day?.offers)&&Array.isArray(catalog))return day.offers.map(o=>{const c=catalog[o[0]]||[];return{name:c[0]||'',slug:c[1]||'',rate:Number(o[1])}});return[]}
  function indexedMatches(day,shop,catalog){
    ...
    for(const o of offers){const c=catalog?.[o[0]]||[],name=c[0]||'',slug=(c[1]||'').toLowerCase(),rate=Number(o[1]),row={name,slug,rate};if(qslug&&slug===qslug){exactSlug.push(row);continue}if(!qslug&&q&&norm(name)===q){exactName.push(row);continue}if(!qslug&&q){const n=norm(name);if(n&&(n.includes(q)||q.includes(n)))partial.push(row)}}
  ...
  function matchRows(day,shop,quality,catalog){
    ...
    const q=norm(shop),qslug=slugFromInput(shop);if(!q&&!qslug)return{rate:null,state:'empty'};const rows=rowsFor(day,catalog);let hits=qslug?rows.filter(s=>(s.slug||'').toLowerCase()===qslug):[];if(!hits.length&&q)hits=rows.filter(s=>norm(s.name)===q);if(!hits.length&&q){const partial=rows.filter(s=>{const n=norm(s.name);return n&&(n.includes(q)||q.includes(n))});const identities=new Set(partial.map(s=>s.slug||norm(s.name)));if(identities.size>1)return{rate:null,state:'ambiguous',matches:partial.slice(0,8)};hits=partial}if(hits.length){hits.sort((a,b)=>Number(b.rate)-Number(a.rate));return{rate:Number(hits[0].rate),state:'match',store:hits[0],quality}}return quality==='ok'?{rate:0,state:'not_found'}:{rate:null,state:'uncertain'}
  }
  ...
  return {norm,slugFromInput,rowsFor,indexedMatches,matchRows,shopRate,safeCampaignTitle,eventsFor,fmtRate};
});
